 ...
 *
 *  One more important thing to note here is that `cancel` DOES NOT stop code execution, it merely ends the request,
 ...
 *
 *  Any error occurring inside the `errorHandler` function will not be detected nor catch by the framework
 * */
...
function middleware(req, res, next, cancel) {
    cancel("Cancelled because of something", (req) => console.log('request cancelled'))
    next()
}
