...
class Routine {
    ...
    constructor(conf) {
        ...
        if (conf.asyncErrorHandler && typeof conf.asyncErrorHandler === 'function') {
            this.conf.asyncErrorHandler = conf?.asyncErrorHandler
        } else if (conf.asyncErrorHandler && typeof conf.asyncErrorHandler !== 'function') {
            console.error(
    ...
    Listen(PORT = 8080, handler = null) {
        ...
        let server = http.createServer(async (req, res) => {
            ...
            await executeMiddlewareHandler(req, res, this.middlewares, parsedUrl, this.conf.asyncErrorHandler)
            ...
            if (route !== undefined) {
                ...
                if (['PUT', 'PATCH', 'POST'].indexOf(req.method) !== -1) {
                    if (
                        ...
                        'multipart/form-data' && !this.conf.allowMultipart
                    ) {
                        res.status(500).json({
                            message:
                    ...
                    await executeRouteHandlers(route, req, res, this.conf.asyncErrorHandler)
